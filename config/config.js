...
var home = process.env.INSIGHT_DB || (getUserHome() + '/.mxtsight');
...
if (!dataDir) {
  if (isWin) dataDir = '%APPDATA%\\Martexcoin\\';
  if (isMac) dataDir = process.env.HOME + '/Library/Application Support/Martexcoin/';
  if (isLinux) dataDir = process.env.HOME + '/.MartexCoin/';
}
dataDir += network === 'testnet' ? 'testnet' : '';

var safeConfirmations = process.env.INSIGHT_SAFE_CONFIRMATIONS || 3;
var ignoreCache = process.env.INSIGHT_IGNORE_CACHE || 0;

var bitcoindConf = {
  protocol: process.env.BITCOIND_PROTO || 'http',
  user: process.env.BITCOIND_USER || 'user',
  pass: process.env.BITCOIND_PASS || 'JO1VAa5f6SMQt97',
